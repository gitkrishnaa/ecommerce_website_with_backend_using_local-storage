...
let source="https://cdn.jsdelivr.net/gh/gitkrishnaa/ecommerce_website_with_backend_using_local-storage@main/image/"+imageNumber+".webp";
...
Filter_price_button.addEventListener("click",()=>{
  let abc=document.querySelectorAll(".div_1_can_none")[0]
  abc.Se
  console.log(abc)
  let filter_data=data.filter((a,i)=>{return data[i].price>600 && data[i].price<700});
console.log(filter_data)
filter_data.map((a,i)=>{
  if(false){
    render_data(data[i].name,data[i].price,data[i].rate,data[i].image_Location)
    }
  render_data(filter_data[i].name,filter_data[i].price,filter_data[i].rate,filter_data[i].image_Location)
...
//filter applay
let filter_data=data.filter((a,i)=>{return data[i].price>500 && data[i].price<600});
console.log(filter_data)
filter_data.map((a,i)=>{
  if(false){
    render_data(data[i].name,data[i].price,data[i].rate,data[i].image_Location)
